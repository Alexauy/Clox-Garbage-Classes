...
static bool errNative(int argCount, Value* args, Value* result) {
  (void)argCount;
  (void)args;
  *result = OBJ_VAL(copyString("Error!", 6));
  return false;
}
...
static bool lenNative(int argCount, Value* args, Value* result){
  ...
  if(!IS_STRING(args[0])){
    *result = OBJ_VAL(copyString("Argument to len() must be a string.", 35));
    return false;
  }
